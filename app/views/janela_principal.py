...
# Estilo global compartilhado por todas as telas
ESTILO_GLOBAL = """
    * {
    ...
    QTableWidget {
        ...
        font-size: 13px;
        color: #c8dff5;
        alternate-background-color: #0a1428;
        selection-background-color: #0d2540;
    ...
    QLineEdit::placeholder { color: #1a3a5a; }
    QComboBox {
        background-color: #080f1e;
        border: 1px solid #0a1e34;
        border-radius: 6px;
...
class JanelaPrincipal(QMainWindow):
    ...
    def _wrapper(self, widget):
        """Envolve um widget em um scroll com padding."""
        container = QWidget()
    ...
    def _criar_pagina_dashboard(self):
        ...
        self.card_abertas      = self._card("Abertas",       "0", "#4a9eff", "OS em aberto aguardando atendimento")
        self.card_andamento    = self._card("Em Andamento",  "0", "#f0a030", "OS sendo executadas agora")
        self.card_concluidas   = self._card("Concluidas",    "0", "#2ab87a", "OS finalizadas com sucesso")
        self.card_clientes     = self._card("Clientes",      "0", "#8a6aff", "Total de clientes cadastrados")
        self.card_equipamentos = self._card("Equipamentos",  "0", "#e05a9a", "Equipamentos registrados")
        ...
        self.card_receita_total    = self._card("Receita Total",    "R$ 0,00", "#4a9eff",  "Soma de todos os servicos")
        self.card_receita_recebida = self._card("Receita Recebida", "R$ 0,00", "#2ab87a",  "Pagamentos confirmados")
        self.card_a_receber        = self._card("A Receber",        "R$ 0,00", "#f0a030",  "Pagamentos pendentes")
        self.card_os_pagas         = self._card("OS Pagas",         "0",       "#8a6aff",  "Ordens com pagamento confirmado")
        for c in [self.card_receita_total, self.card_receita_recebida,
                  self.card_a_receber, self.card_os_pagas]:
            linha2.addWidget(c)
        layout.addLayout(linha2)

        # Linha 3: calendario + OS recentes
        layout.addSpacing(8)
        linha3 = QHBoxLayout()
        ...
        # Calendario
        frame_cal = QFrame()
        frame_cal.setObjectName("card_resumo")
        lc = QVBoxLayout(frame_cal)
        lc.setContentsMargins(20, 16, 20, 16)
        lc.setSpacing(10)
        label_cal = QLabel("CALENDARIO")
        ...
        linha3.addWidget(frame_cal, stretch=42)

        # OS Recentes
        frame_recentes = QFrame()
        frame_recentes.setObjectName("card_resumo")
        lr = QVBoxLayout(frame_recentes)
        ...
        self.lista_recentes.setSpacing(8)
        lr.addLayout(self.lista_recentes)
        lr.addStretch()

        linha3.addWidget(frame_recentes, stretch=58)
    ...
    def _card(self, titulo, valor, cor, dica=""):
        card = QFrame()
        ...
        # Linha topo com indicador de cor
        topo = QHBoxLayout()
        dot = QLabel()
        ...
        lv = QLabel(valor)
        lv.setStyleSheet(f"color: {cor}; font-size: 26px; font-weight: 700;")
        layout.addWidget(lv)
    ...
    def _atualizar_recentes(self):
        ...
        for o in ordens:
            item = QFrame()
            item.setStyleSheet("""
                QFrame { background-color: #0a1428; border-radius: 6px;
                         border: 1px solid #0a1e34; }
                QFrame:hover { border-color: #1a4a7a; }
            """)
            li = QHBoxLayout(item)
    ...
    def _navegar(self, indice: int):
        ...
        todos = self.botoes_menu
        for i, btn in enumerate(todos):
